...
public class GuideContainer extends AbsoluteLayout {
    ...
    private String Tag="GuideContainer";
     ...
     */
    public void addBlackRect(View view, View above, View below, int border, int borderForHight, GuideView.OnClickListener listener) {
        Log.i(Tag, "addBlackRect");
        int[] loc = new int[2];
        viewsList.add(null);
        view.getLocationOnScreen(loc);
        RectF rectF = new RectF(loc[0], loc[1], loc[0] + view.getWidth(), loc[1] + view.getHeight());
        guideView.addRect(rectF, view.getId(), border, borderForHight,listener);
        if (above != null) {
    ...
    public void addBlackRect(View view, View above, View below, int border, GuideView.OnClickListener listener) {
        ...
        view.getLocationOnScreen(loc);
        RectF rectF = new RectF(loc[0], loc[1], loc[0] + view.getWidth(), loc[1] + view.getHeight());
        guideView.addRect(rectF, view.getId(), border, border,listener);
        if (above != null) {
            addAboveToId(above, view.getId());
            viewsList.add(above);
        }
     ...
     * 动态显示
     */
    public void animIn(final int time) {
        Log.i(Tag, "animIn");
        isShow = true;
                    ...
                    for (int j = 0; j < views.size(); ++j) {
                        ...
                        views.get(j).postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                views.get(finalJ).startAnimation(animation1);
                            }
                        }, allCount * time);
                    }
                }else {
